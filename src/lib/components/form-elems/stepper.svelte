<script>
    ...
    $: {
    setTimeout(() => {
        if (!circles || !progress) return;
        // Get all .circle elements inside the container
        const circleNodes = circles.querySelectorAll('.circle');
        ...
        const actives = circles.querySelectorAll('.active');
        progress.style.width = (actives.length - 1) / (circleNodes.length - 1) * 100 + '%';
    });
}
</script>
